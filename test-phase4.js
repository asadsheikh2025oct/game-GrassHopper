...
window.phase4 = function () {
  ...
  // ================= 28 options =================
  {
    ...
    rec('Options', '28 left-handed layout toggles, swaps sides, persists', l1.on && l1.label === 'Left-handed' && l1.cls && l1.saved === '1' && leftRight === '16px' && rightLeft === '16px' && !l2.on && !l2.cls, `on: ${JSON.stringify(l1)}, css right/left=${leftRight}/${rightLeft}, off again=${!l2.on}`);
    ...
    rec('Options', '28 large buttons toggles, applies, persists', b1.on && b1.cls && b1.saved === '1' && b1.w === '104px' && !settings.bigButtons, `${JSON.stringify(b1)}`);
